...
namespace FMOD.Core
{
	...
	public partial class DspConnection : HandleBase
	{
		...
		/// </value>
		/// <remarks>
		/// <para>A <see cref="DspConnection"/> joins two <see cref="Dsp.AddInput"/> units together (think of it as the line between two circles).</para>
		/// <para>Each <see cref="Dsp"/> has one input and one output.</para>
		/// <alert class="note">
		/// <para>If a <see cref="Result.NotReady"/> just occurred, the connection might not be ready because the <see cref="Dsp.AddInput"/> system is still queued to connect in the background. If so the function will return <see cref="Dsp"/> and the input will be <c>null</c>. Poll until it is ready.</para>
		/// </alert>
		/// </remarks>
		...
		/// <para>Each <see cref="Dsp"/> has one input and one output.</para>
		/// <alert class="note">
		/// <para>If a <see cref="Result.NotReady"/> just occurred, the connection might not be ready because the <see cref="Dsp.AddInput">DSP</see> system is still queued to connect in the background.</para>
		/// <para>If so the function will return <see cref="Dsp"/> and the input will be <c>null</c>. Poll until it is ready.</para>
		/// </alert>
		/// </remarks>
		/// <seealso cref="DspConnection"/>
		/// <seealso cref="Dsp"/>
		...
		/// <param name="inChannelHop">Number of floating point values stored in memory for a row, so that the memory can be skipped through correctly to read the right values, if the intended matrix memory to be read from is wider than the matrix stored in the <see cref="Dsp"/>. </param>
